...
class _AddEditCreditNoteScreenState extends ConsumerState<AddEditCreditNoteScreen> {
  ...
  List<CreditNoteItem> _draftItems = [];
  ...
  void _initValues() async {
    ...
    if (widget.parentCreditNoteUuid != null && widget.parentCreditNoteUuid!.isNotEmpty) {
      CreditNote? creditNote = await isar.creditNotes.filter().uuidEqualTo(widget.parentCreditNoteUuid).findFirst();
      if (creditNote == null) {
        final idVal = int.tryParse(widget.parentCreditNoteUuid!);
        if (idVal != null) {
      ...
      if (creditNote != null) {
        ...
        if (creditNote.partyId != null && creditNote.partyId! > 0) {
          party = await isar.partys.get(creditNote.partyId!);
        }
        if (party == null && creditNote.partyName != null && creditNote.partyName!.isNotEmpty) {
          party = await isar.partys.filter().partyNameEqualTo(creditNote.partyName!).findFirst();
        }
        if (party == null) {
          try { await creditNote.party.load(); } catch (_) {}
          try { party = creditNote.party.value; } catch (_) {}
        }
        _selectedParty = party ?? (Party()
          ..partyName = creditNote.partyName
          ..gstNumber = creditNote.gstNumber
          ..addressLine1 = creditNote.address);

        List<CreditNoteItem> itemsList = [];
        ...
        if (itemsList.isEmpty) {
          ...
          try { itemsList = creditNote.creditNoteItems.toList(); } catch (_) {}
        }
        ...
        _draftItems = List<CreditNoteItem>.from(itemsList);
        _recalculateTotals();
        if (mounted) {
          setState(() {});
        }
  ...
  void _saveBill() async {
    ...
    if (_draftItems.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Please add at least one item to creditNote.')),
      );
      return;
    }
    ...
    try {
      ...
      if (_existingCreditNote == null) {
        creditNote.uuid ??= Uuid().v4();
        creditNote.creditNoteNumber = _billNumberController.text.trim();
        creditNote.createdAt = DateTime.now();
        creditNote.version = 1;
      } else {
        creditNote.uuid ??= Uuid().v4();
        creditNote.version = _existingCreditNote!.version + 1;
      }

      creditNote
        ..creditNoteDate = _creditNoteDate
        ..creditNoteNumber = _billNumberController.text.trim()
        ..originalInvoiceNumber = _originalBillNumberController.text.trim()
      ...
      final repo = ref.read(creditNoteRepositoryProvider);
      await repo.saveCreditNote(creditNote, _draftItems);
      final success = true;
      ...
      if (success && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('CreditNote invoice saved successfully!')),
        );
        Navigator.pop(context);
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to save creditNote: $e')),
      );
    } finally {
        ...
        child: Padding(
          ...
          child: Column(
            ...
            TextFormField(
              ...
              decoration: InputDecoration(labelText: 'Bill Level Discount (₹)', border: (ref.watch(themeProvider).themeType == ThemeType.neumorphism) ? OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none) : const OutlineInputBorder(),),
              onChanged: (val) => _recalculateTotals(),
            ),
            const SizedBox(height: 12),
            TextFormField(
              controller: _remarksController,
              decoration: InputDecoration(labelText: 'Remarks / Notes', border: (ref.watch(themeProvider).themeType == ThemeType.neumorphism) ? OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none) : const OutlineInputBorder(),),
            ),
      ...
      bottomNavigationBar: !isDesktop
        ...
        child: Row(
          ...
          children: [
            ...
            ElevatedButton.icon(
              icon: const Icon(Icons.check_circle_outline),
              label: const Text('Save CreditNote Bill', style: TextStyle(fontWeight: FontWeight.bold)),
              onPressed: _saveBill,
              style: (ref.watch(themeProvider).themeType == ThemeType.neumorphism) ? ElevatedButton.styleFrom(elevation: 0, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12))) : ElevatedButton.styleFrom(
              ...
              children: [
                Expanded(
                  child: InkWell(
                    ...
                    child: InputDecorator(
                      decoration: InputDecoration(labelText: 'CreditNote Date', border: (ref.watch(themeProvider).themeType == ThemeType.neumorphism) ? OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none) : const OutlineInputBorder(), isDense: true),
                ...
                Expanded(
                  child: TextFormField(
                    ...
                    readOnly: true,
                    decoration: InputDecoration(labelText: 'Internal Bill # (Auto)', border: (ref.watch(themeProvider).themeType == ThemeType.neumorphism) ? OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none) : const OutlineInputBorder(), isDense: true),
                  ),
                ),
                const SizedBox(width: 8),
                Expanded(
                  child: TextFormField(
                    ...
                    decoration: InputDecoration(labelText: 'Supplier Invoice #', border: (ref.watch(themeProvider).themeType == ThemeType.neumorphism) ? OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none) : const OutlineInputBorder(), isDense: true),
                  ),
                ),
              ],
            ),
                  ...
                  child: Column(
                    children: [
                        ...
                        children: [
                          ...
                          Expanded(
                            child: TextFormField(
                              ...
                              keyboardType: const TextInputType.numberWithOptions(decimal: true),
                              decoration: InputDecoration(labelText: 'GST %', isDense: true, border: (ref.watch(themeProvider).themeType == ThemeType.neumorphism) ? OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none) : const OutlineInputBorder(),),
                              onChanged: (val) {
                      ...
                      TextFormField(
                        ...
                        decoration: InputDecoration(labelText: 'Expiry Date (e.g. 12/28)', isDense: true, border: (ref.watch(themeProvider).themeType == ThemeType.neumorphism) ? OutlineInputBorder(borderRadius: BorderRadius.circular(12), borderSide: BorderSide.none) : const OutlineInputBorder(),),
                        onChanged: (val) {
