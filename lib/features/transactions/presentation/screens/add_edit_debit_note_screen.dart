...
import 'package:business_sahaj_erp/features/transactions/presentation/providers/purchase_providers.dart';
...
class AddEditDebitNoteScreen extends ConsumerStatefulWidget {
  ...
  const AddEditDebitNoteScreen({Key? key, this.parentDebitNoteUuid}) : super(key: key);
      ...
      if (debitNote != null) {
        ...
        if (pUuid != null && pUuid.isNotEmpty) {
          itemsList = await isar.debitNoteItems
              .filter()
              .parentDebitNoteUuidEqualTo(pUuid)
              .findAll();
        }
        ...
        child: Padding(
          ...
          child: Column(
            ...
            TextFormField(
              ...
              decoration: const InputDecoration(labelText: 'Bill Level Discount (â‚¹)', border: OutlineInputBorder()),
                ...
                Expanded(
                  child: InkWell(
                    ...
                    child: InputDecorator(
                      decoration: const InputDecoration(labelText: 'DebitNote Date', border: OutlineInputBorder(), isDense: true),
                                ...
                                child: ListView.builder(
                                  ...
                                  itemBuilder: (context, index) {
                                    ...
                                    return ListTile(
                                      ...
                                      subtitle: Text(
                                        'Code: ${item.itemCode ?? "N/A"} | Buy: â‚¹${item.buyRate?.toStringAsFixed(2) ?? "0"} | Stock: ${item.currentStock?.toInt() ?? 0}',
                        ...
                        children: [
                          Expanded(
                            child: TextFormField(
                              ...
                              keyboardType: const TextInputType.numberWithOptions(decimal: true),
                              decoration: const InputDecoration(labelText: 'Rate Incl (â‚¹)', isDense: true, border: OutlineInputBorder()),
                              onChanged: (val) {
          ...
          children: [
            ...
            Expanded(
              child: TextFormField(
                controller: _rateExclController,
                keyboardType: const TextInputType.numberWithOptions(decimal: true),
                decoration: const InputDecoration(labelText: 'Rate Excl (â‚¹)', isDense: true, border: OutlineInputBorder()),
                onChanged: (val) {
                  final double? excl = double.tryParse(val);
                  if (excl == null) return;
          ...
          children: [
            Expanded(
              child: TextFormField(
                controller: _discController,
                keyboardType: const TextInputType.numberWithOptions(decimal: true),
                decoration: const InputDecoration(labelText: 'Disc (â‚¹)', isDense: true, border: OutlineInputBorder()),
                onChanged: (val) {
                  final double? discVal = double.tryParse(val);
                  if (discVal != null) {
                    _triggerChanged(disc: discVal);
